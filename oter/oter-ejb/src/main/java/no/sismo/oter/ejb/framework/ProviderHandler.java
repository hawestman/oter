...
public class ProviderHandler {
	...
	private static final Logger logger = LogManager
			.getLogger(ProviderHandler.class.getName());

	/**
	 * Creates a providerhandler where it first up classloads a provider from a
	 * given directory
	 * 
	 * @param provider
	 */
	public ProviderHandler(String provider) {
		...
		// URI("classpath://no.sismo.oter.ejb.providers.impl."+provider));
	...
	public ResponseDataDAO handleRequest(RequestParameterDAO requestParameter) {
		...
		propertiesFile.loadProperties(ProviderHandler.class.getClassLoader()
		...
		if (!useDBInstead) {
			...
			File f = new File(prop.getProperty("ProviderPluginsLocation"));
			pm.addPluginsFrom(f.toURI(), new OptionReportAfter());

			ProviderPlugin plugin = pm.getPlugin(
					ProviderPlugin.class,
					new OptionCapabilities("provider:"
							+ requestParameter.getProvider()));

			if (plugin != null) {

				/**
				 * Attempting to get data from provider, fallback is to get data
				 * from database
				 */
				try {
					// Getting data
				...
				// TODO:Replace with log-line
				System.out
						.println("using database because we couldnt get plugin");
